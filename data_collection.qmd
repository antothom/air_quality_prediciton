...
```{r Illustration air quality and traffic sensors}
#| message: false
#| warning: false
#| echo: false
...
berlin_map <- get_stamenmap(
  bbox = c(left=13.1987, right = 13.5662, bottom = 52.4182, top = 52.6030),
  zoom = 12,
...
points <- traffic_detectors %>%
  ...
  select(lng, lat) %>%
  mutate(type = "Air Quality Station"))
...
ggmap(berlin_map) +
  ...
  theme_bw() +
  ggtitle("Air quality stations and traffic detectors in Berlin") +
