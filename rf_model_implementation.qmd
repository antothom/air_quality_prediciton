---
title: "Random Forest Model Implementation"
format: html
editor: source
---
...
create_rf <- function(df, dependent_var, forecast = F) {
...
```{r}
create_rf(airquality_data_by_station$messwerte[[7]], "caqi_index", F)
```
...
xgb_caqi_index_pred_2 <- map(.x = airquality_data_by_station$messwerte, .f = create_rf, "caqi_index", F) %>%
  bind_rows()
...
xgb_no2_pred <- map(.x = airquality_data_by_station$messwerte, .f = create_rf, "NO2", F) %>%
  bind_rows() %>%
  na.omit()
...
xgb_o3_pred <- map(.x = airquality_data_by_station$messwerte, .f = create_rf, "O3", F) %>%
  bind_rows() %>%
  na.omit()
...
xgb_pm25_pred <- map(.x = airquality_data_by_station$messwerte, .f = create_rf, "pm25", F) %>%
  bind_rows() %>%
  na.omit()
...
xgb_pm10_pred <- map(.x = airquality_data_by_station$messwerte, .f = create_rf, "pm10", F) %>%
  bind_rows() %>%
  na.omit()
...
xgb_caqi_index_forecast <- map(.x = airquality_data_by_station$messwerte, .f = create_rf, "caqi_index", T) %>%
  bind_rows() %>%
  na.omit()
...
xgb_no2_forecast <- map(.x = airquality_data_by_station$messwerte, .f = create_rf, "NO2", T) %>%
  bind_rows() %>%
  na.omit()
...
xgb_o3_forecast <- map(.x = airquality_data_by_station$messwerte, .f = create_rf, "O3", T) %>%
  bind_rows() %>%
  na.omit()
...
xgb_pm25_forecast <- map(.x = airquality_data_by_station$messwerte, .f = create_rf, "pm25", T) %>%
  bind_rows() %>%
  na.omit()
...
xgb_pm10_forecast <- map(.x = airquality_data_by_station$messwerte, .f = create_rf, "pm10", T) %>%
  bind_rows() %>%
  na.omit()
